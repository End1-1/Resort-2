...
void RDesk::recover()
{
    ...
    QString rrId = uuid(VAUCHER_RECOVER_N, fAirDb);
    fDbBind[":f_id1"] = rrId;
...
void RDesk::closeOrder(int state)
{
    ...
    DatabaseResult dr;
    dr.select(fDb, "select f_tax from o_Header where f_id=:f_id", fDbBind);
    if (dr.rowCount() > 0) {
        fDbBind[":f_fiscal"] = dr.value("f_tax").toInt();
...
{
    CI_InvoiceItem *ii = CacheInvoiceItem::instance()->get(fHall->fItemIdForInvoice);
    if (!ii) {
        message_error(tr("No invoice item defined for this station"));
        return false;
    }
    CI_TaxMap *tm = CacheTaxMap::instance()->get(ii->fCode);
    if (!tm) {
    ...
    for (int i = 0; i < ui->tblOrder->rowCount(); i++) {
        ...
        fDbBind[":f_amountCash"] = cashMode == tax_mode_cash ? fTable->fAmount : 0;
        fDbBind[":f_amountCard"] = cashMode == tax_mode_cash ? 0 : fTable->fAmount;
...
void RDesk::checkOrderHeader(TableStruct *t, DishStruct *od, bool skipService)
{
    ...
    if (!skipService) {
        if (od) {
            if (h->fDefaultSvcValue > 0.001) {
                ...
                CI_Dish *cd = CacheDish::instance()->get(h->fServiceItem);
                if (!cd) {
    ...
    if (t->fOrder.isEmpty()) {
        ...
        QString query = QString("select f_id from r_table where f_id='%1' for update")
                .arg(t->fId);
        fDb.queryDirect(query);
        t->fOrder = uuid(VAUCHER_POINT_SALE_N + Hall::getHallById(fTable->fHall)->fPrefix, fAirDb);
...
void RDesk::printReceipt(bool printModePayment)
{
    ...
    DatabaseResult drTax;
    fDbBind[":f_order"] = fTable->fOrder;
    fAirDb.open();
    drTax.select(fAirDb, "select f_replyTaxCode from tax_print where f_order=:f_order and f_replyResult=0", fDbBind);
    fAirDb.close();
    ...
    if (drTax.rowCount() > 0) {
        ...
        top += ps->addTextRect(new PTextRect(160, top, 200, rowHeight, drTax.value("f_replyTaxCode").toString(), &th, f))->textHeight();
    }
...
void RDesk::checkEmpty()
{
    ...
    if (orderEmpty) {
        ...
        DatabaseResult dr;
        fDbBind[":f_header"] = fTable->fOrder;
        fDbBind[":f_s1"] = DISH_STATE_REMOVED_NOSTORE;
        fDbBind[":f_s2"] = DISH_STATE_REMOVED_STORE;
        dr.select(fDb, "select * from o_dish where f_header=:f_header and (f_state=:f_s1 or f_state=:f_s2)", fDbBind);
        if (dr.rowCount() > 0) {
            fShowRemoved = true;
            printReceipt(true);
        }
...
void RDesk::on_btnPayment_clicked()
{
    ...
    case 6: {
        ...
        if (d->exec() == QDialog::Accepted) {
            ...
            invoice = fDbRows.at(0).at(0).toString();
            ...
            foreach_rows {
                if (Hall::fHallMap[fTable->fHall]->fItemIdForInvoice == it->at(0).toInt()) {
                    side = 1;
                    break;
                }
            }
    ...
    case 7: {
        ...
        if (d->exec() == QDialog::Accepted) {
            ...
            fDbBind[":f_cityLedger"] = d->fReservationId;
            fDb.update("o_header", fDbBind, where_id(ap(fTable->fOrder)));
    ...
    fDbBind[":f_side"] = side;
    fDbBind[":f_cash"] = fTable->fPaymentMode == PAYMENT_CARD || fTable->fPaymentMode == PAYMENT_CARD || fTable->fPaymentMode == PAYMENT_BANK;
    fDb.update("m_register", fDbBind, where_id(ap(fTable->fOrder)));
