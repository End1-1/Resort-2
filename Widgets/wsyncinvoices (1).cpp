...
void WSyncInvoices::loadInvoice(const QString &id)
{
    ...
    //fReservGuests.select(fDb, "select * from f_reservation_guests where f_reservation=:f_reservation", fDbBind);
...
//    sdbBind[":f_author"] = ui->leUserCode->text();
//    sdbBind[":f_checkInUser"] = ui->leUserCode->text();
//    sdbBind[":f_checkOutUser"] = ui->leUserCode->text();
//    sdbBind[":f_lastEdit"] = ui->leUserCode->text();
    ...
    if (result) {
        ...
        if (maxDate.daysTo(maxDate2) > 1) {
            message_info_tr("Difference between departure date and last charge more the one!");
        }
    } else {
        fDb.fDb.rollback();
        fSDb.fDb.rollback();
