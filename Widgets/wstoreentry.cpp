...
void WStoreEntry::load(int doc)
{
    ...
    for (int i = 0; i < dr.rowCount(); i++) {
        ...
        if (dr.value(i, "f_qty").toDouble() > 0.1) {
            ui->tblData->lineEdit(i, 4)->setDouble(dr.value(i, "f_amount").toDouble() / dr.value(i, "f_qty").toDouble());
        } else {
            ui->tblData->lineEdit(i, 4)->setDouble(0);
        }
        ui->tblData->lineEdit(i, 5)->setDouble(dr.value(i, "f_amount").toDouble());
    }
}
