...
void RKeyboard::on_btnList_clicked()
{
    QStringList mods;
    for (QList<QMap<QString, QString> >::const_iterator it = DishesTable::fMods.begin(); it != DishesTable::fMods.end(); it++) {
        mods.append(it->value(fCurrentLanguage));
    }
    RModifiers *m = new RModifiers(this);
    m->setModifiers(mods);
    if (m->exec() == QDialog::Accepted) {
        emit textChanged(m->mod());
    }
    delete m;
}
